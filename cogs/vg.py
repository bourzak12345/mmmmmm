...
class vg(commands.Cog):
  ...
  async def vg(self, ctx, user_id: int = None):
    allowed_roles = [
    ...
    girl = 1184605496473301072
    role_girl = nextcord.utils.get(ctx.guild.roles, id=girl)
    ...
    if role in user.roles:
      ...
      embed.set_footer(text="Late Night  Verificationn", icon_url=user.avatar.url)
      ...
      vf_message = Embed(
          description=f" ✅ **{user.display_name}** has been verified as female",
          color=Color.green())
